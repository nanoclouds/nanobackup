    ...
    <MainLayout title="Jobs de Backup" subtitle="Configure e gerencie seus backups agendados">
      ...
      <div className="rounded-lg border border-border bg-card">
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              Array.from({ length: 3 }).map((_, i) => <TableRow key={i}><TableCell colSpan={9}><Skeleton className="h-12 w-full" /></TableCell></TableRow>)
            ) : filtered.length === 0 ? (
              <TableRow><TableCell colSpan={9} className="text-center py-8 text-muted-foreground">Nenhum job encontrado</TableCell></TableRow>
            ) : (
              filtered.map((job) => (
                <TableRow key={job.id} className="border-border table-row-hover">
                  ...
                  <TableCell><div className="flex items-center gap-2"><Database className="h-4 w-4 text-muted-foreground" /><span className="text-sm">{job.postgres_instances?.name || '-'}</span></div></TableCell>
                  <TableCell><div className="flex items-center gap-2"><FolderSync className="h-4 w-4 text-muted-foreground" /><span className="text-sm">{job.ftp_destinations?.name || '-'}</span></div></TableCell>
                  <TableCell>
                    <div className="flex gap-1">
                      <Badge variant="secondary" className="text-xs uppercase">{job.format}</Badge>
                      {job.compression !== 'none' && <Badge variant="muted" className="text-xs uppercase">{job.compression}</Badge>}
                    </div>
                  </TableCell>
                  <TableCell><div className="flex items-center gap-2"><Clock className="h-4 w-4 text-muted-foreground" /><code className="font-mono text-xs text-muted-foreground">{job.schedule}</code></div></TableCell>
                  <TableCell><span className="text-sm text-muted-foreground">{job.last_run ? formatDistanceToNow(new Date(job.last_run), { addSuffix: true, locale: ptBR }) : 'Nunca'}</span></TableCell>
